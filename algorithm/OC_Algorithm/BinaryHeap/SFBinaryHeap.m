 ...
 */
- (NSNumber *)left:(NSNumber *)node{
    NSInteger left = 2*node.integerValue+1;
    return left<self.dataArray.count?@(left):nil;
}
/**
 * how to get the right child of the node
 */
- (NSNumber *)right:(NSNumber *)node{
    NSInteger right = 2*node.integerValue+2;
    return right<self.dataArray.count?@(right):nil;
}
/**
 * how to print the node
 */
- (id)string:(NSNumber *)node{
    return [NSString stringWithFormat:@"(%@)",self.dataArray[node.integerValue]];
}
